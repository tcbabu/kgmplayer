...
//     WriteMessage(buff);
     if(PulseMsg[0]!= '\0' )  WriteMessage(PulseMsg);
     else      WriteMessage(buff);
     return 1;
}
...
#endif
#if 1
         if(SearchString(buff,(char *)"pipewire-pulse")>=0) {
           PULSE=1;
           PIPEWIRE=1;
         }
#endif
...
int CheckPulse(void){
   ...
   if( PULSE) {
     ...
     if(PIPEWIRE==0) runjob(buff,ProcessPacmd);             
//     sprintf(buff,"ps -e");
//     runjob(buff,ProcessPsme);             
   ...
   PulseMsg[0]='\0';
   if( (PULSE==1)&&(PIPEWIRE==0) ) sprintf(PulseMsg,"PULSE yes : PIPEWIRE no..\n");
   if( (PULSE==1)&&(PIPEWIRE==1) ) sprintf(PulseMsg,"PULSE yes : PIPEWIRE yes ..\n");
   if( (PULSE==0)&&(PIPEWIRE==1) ) sprintf(PulseMsg," PIPEWIRE with pipewire-pulse  yes..\n");
...
//  kgRoundedRectangleFill(fid,l*0.5,w*0.5,(float)l-1.,(float)w-1.,0,40,0.1);
  kgTextAngle(fid,90.);
  kgTextSize(fid,l*0.5,l*0.35,l*0.05);
  kgTextFont(fid,23);
  kgTextColor(fid,0);
  kgMove2f(fid,l*0.55+1,(float)l*0.55+1);
  kgTextAngle(fid,90.);
  kgWriteText(fid,(char *)"kgMplayer");
  kgTextColor(fid,15);
  kgMove2f(fid,l*0.55,(float)l*0.55-1);
  kgTextAngle(fid,90.);
  kgWriteText(fid,(char *)"kgMplayer");
  kgChangeColor(fid,30,60,80,65);
  kgTextColor(fid,30);
  kgMove2f(fid,l*0.55,(float)l*0.55);
  kgTextAngle(fid,90.);
  kgWriteText(fid,(char *)"kgMplayer");
  kgTextSize(fid,l*0.3,l*0.16,0.0);
