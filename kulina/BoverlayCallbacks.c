...
// pt[0] is args passed as inputs; pt[1] is output pointer
  ...
  T = (DIT *)kgGetNamedWidget(Tmp,(char *)"OBinput");
  FileName[0]='\0';
  if(!FolderBrowser(FileName))return 0;
  kgSetString(T,0,FileName);
  kgUpdateWidget(T);
  TO = (DIT *)kgGetNamedWidget(Tmp,(char *)"OBout");
...
void  BoverlayOBinputbrowseinit (DIN *B,void *ptmp) {
...
void  BoverlayOBgoinit (DIL *B,void *ptmp) {
 void **pt=(void **)ptmp; //pt[0] is arg 
// may use kgChangeButtonNormalImage etc...
 BUT_STR *buts;
 buts = (BUT_STR *) (B->buts);
...
void  BoverlayOBOutbrowseinit (DIN *B,void *ptmp) {
 void **pt=(void **)ptmp; //pt[0] is arg 
// may use kgChangeButtonNormalImage etc...
 BUT_STR *buts;
 buts = (BUT_STR *) (B->buts);
...
void * BoverlayCleanDia(void *args) {
  /*********************************** 
    args :  Pointer to args  
   ***********************************/ 
...
void *  BoverlayAction(void *Tmp,void *Args) {
  return NULL;
} 
 
 
int   BoverlayOn(void *itmp) {
...
void * BoverlayInterface(void *args,void *rets) {
  /*********************************** 
   ***********************************/ 
  ...
  it->ysh=0;
  it->RunDia = RunBoverlay;
  it->MakeGroup = MakeBoverlayGroup;
  it->Title = GetPointer((char *)"Boverlay");
  it->Help = GetPointer( (char *)"No help yet, request");
  it->Action = BoverlayAction;
  it->Settings = BoverlaySetup;
  it->Cleanup  = BoverlayCleanDia;
  if(args != NULL) Args=args;
  if(rets != NULL) Rets=rets;
  it->args = Args;
  it->rets = Rets;
  it->SwitchOn = BoverlayOn;
  it->SwitchOff = BoverlayOff;
