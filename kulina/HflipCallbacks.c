...
int HflipHFLIPinputcallback(int cellno,int i,void *Tmp) {
  ...
  D = (DIALOG *)Tmp;
  T = (DIT *)kgGetNamedWidget(Tmp,(char *)"HFLIPinput");
  TO = (DIT *)kgGetNamedWidget(Tmp,(char *)"HFLIPout");
...
void  HflipHFLIPinputbrowseinit (DIN *B,void *ptmp) {
...
int HflipHFLIPgocallback( int butno,int i,void *Tmp) {
  ...
  n = B->nx;
  DII *InfoBox = (DII *)kgGetNamedWidget(Tmp,(char *)"HFLIPIbox");
  char infile[300],outfile[300];
  DIT *T = (DIT *)kgGetNamedWidget(Tmp,(char *)"HFLIPinput");
  DIT *TO = (DIT *)kgGetNamedWidget(Tmp,(char *)"HFLIPout");
  infile[0]='\0';
...
void  HflipHFLIPgoinit (DIL *B,void *ptmp) {
 void **pt=(void **)ptmp; //pt[0] is arg 
// may use kgChangeButtonNormalImage etc...
 BUT_STR *buts;
 buts = (BUT_STR *) (B->buts);
...
// pt[0] is args passed as inputs; pt[1] is output pointer
  ...
  if(kgFolderBrowser(Tmp,200,200,File,(char *)"*")) {
    DIT *TO=(DIT *)kgGetNamedWidget(Tmp,"HFLIPout");
...
void  HflipHFLIPOutbrowseinit (DIN *B,void *ptmp) {
 void **pt=(void **)ptmp; //pt[0] is arg 
// may use kgChangeButtonNormalImage etc...
 BUT_STR *buts;
 buts = (BUT_STR *) (B->buts);
 ...
 /* Callback for  HFLIPsec   */ 

int HflipHFLIPseccallback(int cellno,int i,void *Tmp) {
  /************************************************* 
   cellno: current cell counted along column strting with 0 
           ie 0 to (nx*ny-1) 
   i     : widget id starting from 0 
...
void *  HflipAction(void *Tmp,void *Args) {
  return NULL;
} 
 
 
int   HflipOn(void *itmp) {
...
void * HflipInterface(void *args,void *rets) {
  /*********************************** 
   ***********************************/ 
  ...
  it->ysh=0;
  it->RunDia = RunHflip;
  it->MakeGroup = MakeHflipGroup;
  it->Title = GetPointer((char *)"Hflip");
  it->Help = GetPointer( (char *)"No help yet, request");
  it->Action = HflipAction;
  it->Settings = HflipSetup;
  it->Cleanup  = HflipCleanDia;
  if(args != NULL) Args=args;
  if(rets != NULL) Rets=rets;
  it->args = Args;
  it->rets = Rets;
  it->SwitchOn = HflipOn;
  it->SwitchOff = HflipOff;
