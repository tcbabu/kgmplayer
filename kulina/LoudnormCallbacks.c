...
int RunLoudnormbkgr(char *job,int (*ProcessOut)(int,int,int),int (*function)(int,char **),char *infile,char *outfile) ;
int RunLoudnormbkgr_o(char *job,int (*ProcessOut)(int,int,int),int (*function)(int,char **),char *infile,char *outfile) ;
...
int  Loudnormbutton1callback(int butno,int i,void *Tmp) {
  ...
  n = B->nx*B->ny;
  X1 = (DIX *)kgGetNamedWidget(Tmp,"LXbox");
...
int runloudfunction(char *job,int (*ProcessOut)(void *,int,int),int (*function)(int,char **),char *arg){
   int ret =0;
   FILE *fp,*fp1;
...
//  meanlevel = kgGetInt(T1,0);
  ...
  if((th != NULL)&&(th[0]!=NULL)) {
	  ...
	  while(th[j] != NULL) {
            ...
            DoInAnyThread(Thds,RunVolumeLoudnorm,vstr);
	    j++;
	  }
	  WaitThreads(Thds);
	  CloseThreads(Thds);
