...
// pt[0] is args passed as inputs; pt[1] is output pointer
  ...
  DIT *TO = (DIT *)kgGetNamedWidget(Tmp,(char *)"AREVoutput");
...
// pt[0] is args passed as inputs; pt[1] is output pointer
  ...
  DIT *T,*TO;
  char FileName[300],OutFile[300];
  T = (DIT *)kgGetNamedWidget(Tmp,(char *)"AREVinput");
  TO = (DIT *)kgGetNamedWidget(Tmp,(char *)"AREVoutput");
  n = B->nx*B->ny;
  FileName[0]='\0';
  strcpy(FileName,kgGetString(T,0));
  strcpy(OutFile,kgGetString(TO,0));
...
void  AudioReverseAREVIbrowseinit (DIN *B,void *ptmp) {
...
// pt[0] is args passed as inputs; pt[1] is output pointer
  ...
  n = B->nx;
  DIT *T=(DIT *)kgGetNamedWidget(Tmp,(char *)"AREVinput");
  DIT *TO=(DIT *)kgGetNamedWidget(Tmp,(char *)"AREVoutput");
  DII *I  = (DII *)kgGetNamedWidget(Tmp,(char *)"AREVinfo");
  ...
  char buff[500];
  ret =0;
  sprintf(buff,"ffmpegfun -y -i %s   -vn  -af areverse   %s",
  kgGetString(T,0),kgGetString(TO,0));
  kgWrite(I,buff);
  RunMonitorAndWait(buff);
  sprintf(buff,"FINISHED JOB....");
  kgWrite(I,buff);
 
  return ret;
}
void  AudioReverseAREVdoinit (DIL *B,void *ptmp) {
 void **pt=(void **)ptmp; //pt[0] is arg 
// may use kgChangeButtonNormalImage etc...
 BUT_STR *buts;
 buts = (BUT_STR *) (B->buts);
...
void  AudioReverseAREVObrowseinit (DIN *B,void *ptmp) {
 void **pt=(void **)ptmp; //pt[0] is arg 
// may use kgChangeButtonNormalImage etc...
 BUT_STR *buts;
 buts = (BUT_STR *) (B->buts);
...
void * AudioReverseCleanDia(void *args) {
  /*********************************** 
    args :  Pointer to args  
   ***********************************/ 
...
void *  AudioReverseAction(void *Tmp,void *Args) {
  return NULL;
} 
 
 
int   AudioReverseOn(void *itmp) {
...
void * AudioReverseInterface(void *args,void *rets) {
  /*********************************** 
   ***********************************/ 
  ...
  it->ysh=0;
  it->RunDia = RunAudioReverse;
  it->MakeGroup = MakeAudioReverseGroup;
  it->Title = GetPointer((char *)"AudioReverse");
  it->Help = GetPointer( (char *)"No help yet, request");
  it->Action = AudioReverseAction;
  it->Settings = AudioReverseSetup;
  it->Cleanup  = AudioReverseCleanDia;
  if(args != NULL) Args=args;
  if(rets != NULL) Rets=rets;
  it->args = Args;
  it->rets = Rets;
  it->SwitchOn = AudioReverseOn;
  it->SwitchOff = AudioReverseOff;
...
int AudioReverseAREVradiocallback(int item,int i,void *Tmp) {
  /*********************************** 
...
void  AudioReverseAREVradioinit (DIRA *R,void *ptmp) {
 void **pt=(void **)ptmp; //pt[0] is arg 
}
