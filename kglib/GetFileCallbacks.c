...
int  GetFilebrowser2callback(int item,int i,void *Tmp) {
  ...
  str = kgGetSelectedString(Y);
  if(str != NULL) {
	  strcpy(SelectFile,Folder);
	  if(strlen(Folder) > 1)strcat(SelectFile,"/");
	  strcat(SelectFile,str);
	  kgWrite(M1,SelectFile);
	  kgUpdateOn(Tmp);
  }
...
int  GetFilesplbutton1callback(int butno,int i,void *Tmp) {
  ...
  switch(butno) {
    case 1: 
      break;
    case 2: 
      if(strlen(SelectFile)> 1) {
      str = (char **)(D->pt);
      *str = (char *) malloc(strlen(SelectFile)+1);
      strcpy(*str,SelectFile);
      }
      break;
  }
  return ret;
...
int GetFileinit(void *Tmp) {
  ...
  D = (DIALOG *)Tmp;
  pt = D->pt;
  T1 = ( DIM * ) kgGetNamedWidget ( Tmp , ( char * ) "FolderMsg" ) ;
