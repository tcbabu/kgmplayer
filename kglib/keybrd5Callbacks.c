...
int kgFixButton(DIN *B,void *pt){
    ...
    int Bodr;
    int Bxoff=2,Byoff=2;
    float Dbx=36,Dby=36,Dxg=2,Dyg=2;
    float Hx,Hy;
    KEYBRD *kb;
//    B->nodrawbkgr = NOBKGR;
    if(pt != NULL) {
...
int  keybrd5button7callback(int butno,int i,void *Tmp) {
  ...
  if(Kbrd->CurWid < 0) {
    ...
    if(Kbrd->TargetWindow != NULL) {
//      kgSetInputFocus(D,Kbrd->TargetWindow);
      if(butno==1) kgSendLeftKeyToWindow(D,Kbrd->TargetWindow);
      else kgSendRightKeyToWindow(D,Kbrd->TargetWindow);
    }
  }
  else {
    kgSetCurrentWidget(D,Kbrd->CurWid);
...
static int SendControlKey(void *Tmp,int ch) {
  int ret=1;
  DIALOG *D;
  KEYBRD *Kbrd;
  D = (DIALOG *)Tmp;
...
//  fprintf(stderr,"Inside:SendControlKey %d:%c\n",ch,ch);
  ...
  if(Kbrd->CurWid < 0){
    ...
    if(Kbrd->TargetWindow != NULL) {
//      kgSetInputFocus(D,Kbrd->TargetWindow);
//      fprintf(stderr,"kgSendControlKeyToWindow\n");
      kgSendControlKeyToWindow(D,Kbrd->TargetWindow,ch);
    }
  }
  else {
    kgSetCurrentWidget(D,Kbrd->CurWid);
    kgSendControlKeyEvent(D,ch);
  }
  return ret;
}
...
int  keybrd5button25callback(int butno,int i,void *Tmp) {
  ...
  if(Kbrd->CurWid < 0){
    ...
    if(Kbrd->TargetWindow != NULL) {
      switch(butno) {
        case 1: 
          kgSendDeleteKeyToWindow(D,Kbrd->TargetWindow);
          break;
        case 2: 
          kgSendInsertKeyToWindow(D,Kbrd->TargetWindow);
          break;
      }
    }
  }
...
//      kgSetInputFocus(D,Kbrd->TargetWindow);
      switch(butno) {
        case 1:
          kgSendPageupKeyToWindow(D,Kbrd->TargetWindow);
          break;
        case 2:
          kgSendPagedownKeyToWindow(D,Kbrd->TargetWindow);
          break;
        case 3:
          kgSendUpKeyToWindow(D,Kbrd->TargetWindow);
          break;
        case 4:
          kgSendDownKeyToWindow(D,Kbrd->TargetWindow);
          break;
      }
    }
  }
