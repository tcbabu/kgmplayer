...
#define fz_inter(x,m,c) (float)(((x)*(m) + c))
  ...
  void ui_3_panel ( DIG *G , float *x , float *y , float *z , int color , int flag , int n ) 
      ...
      for ( i = 0;i < n;i++ ) {
          ...
          y1 [ i ] = uiscr_y ( y [ i ] ) ;
          z1 [ i ] = uifscr_z ( z [ i ] ) ;
      if ( xmin > x1 [ i ] ) xmin = x1 [ i ] ; if ( ymin > y1 [ i ] ) ymin = y1 [ i ] ;
           if ( xmax < x1 [ i ] ) xmax = x1 [ i ] ; if 
          ( ymax < y1 [ i ] ) ymax = y1 [ i ] ; }
      if ( ( xmin == xmax ) || ( ymin == ymax ) ) return;
//  if((xmax < dc->v_x1)||(xmin >dc->v_x2)||(ymax<dc->v_y1)||(ymin >dc->v_y2)) return;
      if ( ( xmax < ( dc->v_x1+dc->D_x ) ) || ( xmin > ( dc->v_x2+dc->D_x ) ) 
          || ( ymax < ( dc->v_y1+dc->D_y ) ) || ( ymin > 
          ( dc->v_y2+dc->D_y ) ) ) return;
...
/*        while (labs(c-f)>0)*/
  ...
  void uidraw_line3 ( DIG *G , int xa , int ya , int za , int xb , int yb , int zb ) 
      ...
      wc = G->wc;
      dc = G->dc;
      if ( dc->ln_width == 1 ) { _uidraw_line3 ( G , xa , ya , za , xb , yb , zb ) ;
          return;};
      ...
      while ( abs ( c-f ) > 0 ) { i = 0;
          while ( ( i <= dc->ln_style ) && ( abs ( c-f ) > 0 ) ) {
              if ( dx1 > dy1 ) {z = z_inter ( c , m , cons ) ;
                  uiput_pix3 ( G , c , ( wc->EVGAY-r ) , z ) ;}
              else {z = z_inter ( r , m , cons ) ;uiput_pix3 
                  ( G , r , ( wc->EVGAY-c ) , z ) ;}
              c+= ch;
              if ( g > 0 ) { r+= rh; g+= inc2; }
              else g+= inc1;
      ...
      if ( ( x_min <= wc->c_v_x2 ) && ( x_max1 >= wc->c_v_x1 ) ) {
          ...
          if ( ( y_min <= wc->c_v_y2 ) && ( y_max >= wc->c_v_y1 ) ) {
              ...
              for ( i = x_min;i <= x_max1;i++ ) {
                  for ( j = y_min;j <= y_max;++j ) {z = z_inter ( x_min , m , cons ) ;
                      uiput_pix3 ( G , x_min , wc->EVGAY-j , z ) ;}
              }
          }
      }
      wcset_clr ( wc , temp ) ;
  ...
  void _uipoly_fill3 ( DIG *G , short n , int *px , int *py , float *pz , short flag , unsigned int b_color ) 
	 ...
	 /*displaying scan lines*/
          for ( i = 0;i < cnt;i+= 2 ) uihori_line3 ( G , xcord [ i ] , xcord [ i+1 ] , scan , 
              ( int ) ( zcord [ i ] +0.5 ) , ( int ) ( zcord [ i+1 ] +0.5 ) ) ;
          scan++;
      }
      wcset_clr ( wc , dc->ln_color ) ;
      if ( flag == 1 ) for ( i = 0;i < n;i++ ) _uidraw_line3 ( G , px [ i ] , py [ i ] , 
          ( int ) ( pz [ i ] +0.5 ) , px [ ( i+1 ) %n ] , py [ 
          ( i+1 ) %n ] , ( int ) ( pz [ ( i+1 ) %n ] +0.5 ) ) ;
  }
  void _uigodard_fill3 ( DIG *G , short n , int *px , int *py , int *pz , short flag , float *pv ) 
	 ...
	 /*displaying scan lines*/
              for ( i = 0;i < cnt;i+= 2 ) uigodr_line3 ( G , xcord [ i ] , xcord [ i+1 ] , scan , zcord [ i ] , zcord [ i+1 ] , pcord [ i ] , pcord [ i+1 ] ) ;
                  
              scan++;
          }
      }
      wcset_clr ( wc , dc->ln_color ) ;
      if ( flag == 1 ) for ( i = 0;i < n;i++ ) _uidraw_line3 ( G , px [ i ] , py [ i ] , pz [ i ] , px [ 
          ( i+1 ) %n ] , py [ ( i+1 ) %n ] , pz [ ( i+1 ) %n ] ) ;
  }
  ...
  void _uishade_fill3 ( DIG *G , short n , int *px , int *py , int *pz , short flag , float *pv ) 
	 ...
	 /*displaying scan lines*/
              for ( i = 0;i < cnt;i+= 2 ) uishade_line3 ( G , xcord [ i ] , xcord [ i+1 ] , scan , zcord [ i ] , zcord [ i+1 ] , pcord [ i ] , pcord [ i+1 ] ) ;
