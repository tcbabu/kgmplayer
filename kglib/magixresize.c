...
void *ResizeImage(void *Simage,int scolumns,int srows,long columns,  long rows,int filter)
{
  ...
  unsigned long *source_image, *resize_image;
  ...
  n = columns*rows;
  resize_image = (unsigned long *)malloc(sizeof(long)*columns*rows);
  if (resize_image == NULL) return NULL;
  source_image = (unsigned long *) Simage;
  if ((columns == scolumns) && (rows == srows) ) {
    for(i=0;i<n;i++) resize_image[i]=source_image[i];
    return resize_image;
  }
  /*
    Allocate filter contribution info.
  */
  order=(((double) columns*(srows+rows)) >
         ((double) rows*(scolumns+columns)));

  x_factor=(double) columns/scolumns;
  y_factor=(double) rows/srows;
  i=(long) filter;
