...
// field is counted from 1
      ...
      int comp_list ( Dlink *L1 , Dlink *L2 , int comprecord 
          ( void *tmp1 , void *tmp2 ) ) ;
...
//
      void* Dpick ( Dlink *LN ) ; /* like Ddelete but does not free */ int Dlocation 
          ( Dlink *L , int pos ) ;
...
#if 0
      / * * Sample code for compare function * LINK MUST BE IN ASCENTING ORDER */ int CompareAction 
          ( void *rec , char *name ) {
	/* 
