...
void Opntwin(DIALOG *D,int ix,int iy,int chrs,int lines,int xbdr,int ybdr)
 {
  ui_txtwinnew(D,(int) ix,(int) iy,(int) chrs,(int) lines,xbdr,ybdr);
 }
...
#else
  kgSplashMessage(D,10,10,strlen(s)*8,22,(char *)s,12,0,15);
#endif
 }
...
void arrange(char **m,int n) {
  ...
  for(i=0;i<n-1;i++) {
    for(j=i+1;j<n;j++){
     if(strcmp(m[i],m[j])>0) {pt=m[j];m[j]=m[i];m[i]=pt;}
    }
  }
}
int Up_Tx_Table(int code,int i,DIA *d){
  ...
  int ret=1;
  D = (DIALOG *)(d[i].t->D);
  if( d[i].t->Update != NULL ) 
      ret = d[i].t->Update(code,i,d);
...
int Up_Bt_Box(int code,int i,DIA *d){
  ...
  int ret=1;
  D = (DIALOG *)(d[i].b->D);
  if( d[i].b->Update != NULL ) 
      ret = d[i].b->Update(code,i,d);
  else if(D->Callback != NULL) ret = D->Callback(D,&(D->kb));
  return ret;
}
int Up_Fsld_Bar(double code,int i,DIALOG *D) {
  ...
  DIA *d;
  d = D->d;
  if( d[i].f->Update != NULL ) 
      ret = d[i].f->Update(code,i,D);
  else if(D->Callback != NULL) ret = D->Callback(D,&(D->kb));
  return ret;
}
int Up_Isld_Bar(int code,int i,DIALOG *D) {
  ...
  DIA *d;
  d = D->d;
  if( d[i].d->Update != NULL ) 
      ret = d[i].d->Update(code,i,D);
  else if(D->Callback != NULL) ret = D->Callback(D,&(D->kb));
  return ret;
}
...
int Up_D_Y_Box(int code,int i,DIALOG *D){
  ...
  DIA *d;
  d = D->d;
  if( d[i].y->Update != NULL ) 
      ret = d[i].y->Update(code,i,D);
  else if(D->Callback != NULL) ret = D->Callback(D,&(D->kb));
  return ret;
}
...
int Up_D_Btn_Box(int code,int i,DIALOG *D){
  ...
  DIA *d;
  d = D->d;
  if( d[i].N->Update != NULL ) 
      ret = d[i].N->Update(code,i,D);
  else if(D->Callback != NULL) ret = D->Callback(D,&(D->kb));
  return ret;
}
int Up_D_SplBt_Box(int code,int i,DIALOG *D){
  ...
  DIA *d;
  d = D->d;
  if( d[i].h->Update != NULL ) 
      ret = d[i].h->Update(code,i,D);
  else if(D->Callback != NULL) ret = D->Callback(D,&(D->kb));
  return ret;
}
...
int Up_D_Brw_Box(int code,int i,DIALOG *D){
  ...
  DIA *d;
  d = D->d;
  if( d[i].w->Update != NULL ) 
      ret = d[i].w->Update(code,i,D);
  else if(D->Callback != NULL) ret = D->Callback(D,&(D->kb));
  return ret;
}
int Up_D_Menu_Box(int code,int i,DIALOG *D){
  ...
  DIA *d;
  d = D->d;
  if( d[i].e->Update != NULL ) 
      ret = d[i].e->Update(code,i,D);
  else if(D->Callback != NULL) ret = D->Callback(D,&(D->kb));
  return ret;
}
int Up_D_NewMenu_Box(int code,int i,DIALOG *D){
  ...
  DIA *d;
  d = D->d;
  if( d[i].x->Update != NULL ) 
      ret = d[i].x->Update(code,i,D);
  else if(D->Callback != NULL) ret = D->Callback(D,&(D->kb));
  return ret;
}
 ...
 *nr = n;
 for(i=0;i<n;i++) {
   ...
   switch(ch) {
     case 't':
     case 'T':
       if( uiCheckClickPosition(d[i].t->x1+x1,d[i].t->y1+y1,d[i].t->x2+x1,d[i].t->y2+y1,D->PON_X,D->PON_Y) ==1)
           return i%n;
       break;
     case 'y':
       
       if( uiCheckClickPosition(d[i].y->x1+x1,d[i].y->y1+y1,d[i].y->x2+x1,d[i].y->y2+y1,D->PON_X,D->PON_Y) ==1)
           return i%n;
       break;
     case 'c':
       if( uiCheckClickPosition(d[i].c->x1+x1,d[i].c->y1+y1,d[i].c->x2+x1,d[i].c->y2+y1,D->PON_X,D->PON_Y) ==1)
           return i%n;
       break;
     case 'r':
       if( uiCheckClickPosition(d[i].r->x1+x1,d[i].r->y1+y1,d[i].r->x2+x1,d[i].r->y2+y1,D->PON_X,D->PON_Y) ==1)
           return i%n;
       break;
     case 'h':
     case 'H':
       if( uiCheckClickPosition(d[i].h->x1+x1,d[i].h->y1+y1,d[i].h->x2+x1,d[i].h->y2+y1,D->PON_X,D->PON_Y) ==1)
           return i%n;
       break;
     case 'b':
     case 'n':
     case 'N':
       if( uiCheckClickPosition(d[i].b->x1+x1,d[i].b->y1+y1,d[i].b->x2+x1,d[i].b->y2+y1,D->PON_X,D->PON_Y) ==1)
           return i%n;
       break;
     case 'f':
       if( uiCheckClickPosition(d[i].f->x1+x1,d[i].f->y1+y1,d[i].f->x2+x1,d[i].f->y2+y1,D->PON_X,D->PON_Y) ==1)
           return i%n;
       break;
     case 'd':
       if( uiCheckClickPosition(d[i].d->x1+x1,d[i].d->y1+y1,d[i].d->x2+x1,d[i].d->y2+y1,D->PON_X,D->PON_Y) ==1)
           return i%n;
       break;
     case 'P':
       if( uiCheckClickPosition(d[i].B->x1+x1,d[i].B->y1+y1,d[i].B->x2+x1,d[i].B->y2+y1,D->PON_X,D->PON_Y) ==1)
       ...
       break;
     case 'w':
       if( uiCheckClickPosition(d[i].w->x1+x1,d[i].w->y1+y1,d[i].w->x2+x1,d[i].w->y2+y1,D->PON_X,D->PON_Y) ==1)
           return i%n;
       break;
     case 'x':
       if( uiCheckClickPosition(d[i].x->x1+x1,d[i].x->y1+y1,d[i].x->x2+x1,d[i].x->y2+y1,D->PON_X,D->PON_Y) ==1)
           return i%n;
       break;
     case 'e':
       if( uiCheckClickPosition(d[i].e->x1+x1,d[i].e->y1+y1,d[i].e->x2+x1,d[i].e->y2+y1,D->PON_X,D->PON_Y) ==1)
           return i%n;
       break;
     case 's':
       if( uiCheckClickPosition(d[i].s->x1+x1,d[i].s->y1+y1,d[i].s->x2+x1,d[i].s->y2+y1,D->PON_X,D->PON_Y) ==1)
           return i%n;
       break;
     case 'v':
       if( uiCheckClickPosition(d[i].v->x1+x1,d[i].v->y1+y1,d[i].v->x2+x1,d[i].v->y2+y1,D->PON_X,D->PON_Y) ==1)
           return i%n;
       break;
     case 'z':
       if( uiCheckClickPosition(d[i].z->x1+x1,d[i].z->y1+y1,d[i].z->x2+x1,d[i].z->y2+y1,D->PON_X,D->PON_Y) ==1)
           return i%n;
       break;
     default:
       break;
       ...
       case 'H':
         {
          ...
          if(h->titles != NULL) {
            n = h->nx*h->ny;
            for(j=0;j<n;j++) if(h->titles[j]!= NULL) Free(h->titles[j]);
            Free(h->titles);
          }
         }
         FreeImg( ((DILN *)(t))->Bimg);
       ...
       case 'b':
         {
          ...
          if(b->titles != NULL) {
            n = b->nx*b->ny;
            for(j=0;j<n;j++) if(b->titles[j]!= NULL) Free(b->titles[j]);
            Free(b->titles);
          }
         }
         FreeImg( ((DIB *)(t))->Bimg);
   ...
   while(d[i].t != NULL) {
      ...
      switch ((int)ch) {
       ...
       case 'N':
         break;
       case 'f':
         *(d[i].f->df) = _ui_getfslidevalue((S_STR *)(d[i].f->sptr));
         break;
       case 'd':
         *(d[i].d->df) =  _ui_getdslidevalue((S_STR *)(d[i].d->sptr));
         break;
       case 'P':
         *(d[i].d->df) =  _ui_getdslidevalue((S_STR *)(d[i].B->sptr));
         break;
       case 'w':
...
// strcpy(D->name,"Kulina Designer");
 sprintf(D->name,"Kulina Designer Ver. %d.%2.2d",((D->VerId%10000)/100),((D->VerId%100)));
...
again:
   ...
   if(D->controls>1){
        ...
        j=0;
        while(d[j].t != NULL) { 
          if((d[j].t->code=='H')||(d[j].t->code=='h')){hcontrols++;break;}
          else j++;
       }
   }
   uiUpdateOn(D);
...
void * kgGetClickedWidget(void *Dtmp) {
 ...
 for(i=0;i<n;i++) {
   if( kgGetWidgetVisibility(kgGetWidget(D,i))==0) continue;
   if( uiCheckClickPosition(d[i].t->x1+xo,d[i].t->y1+yo,d[i].t->x2+xo,
