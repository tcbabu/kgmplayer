...
#include <pthread.h>
extern int Kulina;
static unsigned long MWIN;
extern int FULLSCREEN;
int gnome_layer=0;
pthread_t Pth;
DIALOG *Dum=NULL;
void *RunControls(void *);
void * Mwin(void *,void *,void *);
Window kgGetKulinaWindow(void *);
Display *kgGetKulinaDisplay(void *);
#endif
...
static int CheckEscape(Display *Dsp,XEvent *event) {
  ...
  for(i=K_min;i<=K_max;i++) {
    ...
    if(keysym[k+1]!= NoSymbol) Scan_sh_code[i]= keysym[k+1];
    ...
    if(Kulina) {
        switch (event->type)
        {
          case ConfigureNotify:
                ...
                return check_resize();
          case KeyRelease:
//          key =((XKeyEvent *)event)->keycode;
          pesc=0;
          if(!CheckEscape(mydisplay,event)) return 0;
          else pesc=1;
          case ButtonPress:
//          case ButtonRelease:
//          case KeyRelease:
...
//              RunControls(NULL);
//              pthread_create(&(Pth),NULL, RunControls,Dum);
            }
            if(!pesc) fprintf(stderr,"B:\r");
            else fprintf(stderr,"B: 1\r");
            fflush(stderr);
            return 0;
          default:
           return 0;
                             ...
                             const char *classname, const char *title)
{
  ...
  if (window_state & VOFLAG_HIDDEN) {
    ...
    // wait for map
#ifdef D_KULINA
    Dum= kgUiFromWindow((void *)mDisplay, (void *)vo_window,(void *)col_map);
#endif
    do {
      XNextEvent(mDisplay, &xev);
...
final:
...
#ifdef D_KULINA
  {
    ...
    fprintf(stderr,"W: %d %d\r",MWIN,gnome_layer);
    fflush(stderr);
  }
#endif
}
