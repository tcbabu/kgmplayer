...
if(len>0){
  if(data){
    // DATA
    stream_write_buffer(stream,(unsigned char *) data, len);
    if(len&1){  // padding
      unsigned char zerobyte=0;
      stream_write_buffer(stream,(unsigned char *) &zerobyte, 1);
    }
  } else {
    // JUNK
    ...
    while(len>0){
      ...
      if(l>len) l=len;
      stream_write_buffer(stream,(unsigned char *) avi_junk_data, l);
      len-=l;
    }
  }
}
