...
resample_rate(double freq)
{
    ...
    case 48:
        return 48000;
    default:
        printf("Illegal resample frequency: %.3f kHz\n", freq);
        return 0;
    }
}
...
int generateOutPath(lame_t gfp, char const* inPath, char const* outDir, char* outPath)
{
    ...
    return 0;
err_generateOutPath:
    printf( "error: output file name too long" );
    return 1;
#else
...
set_id3_albumart(lame_t gfp, char const* file_name)
{
    ...
    switch (ret) {
    case 1: printf("Could not find: '%s'.\n", file_name); break;
    case 2: printf("Insufficient memory for reading the albumart.\n"); break;
    case 3: printf("Read error: '%s'.\n", file_name); break;
    case 4: printf("Unsupported image: '%s'.\nSpecify JPEG/PNG/GIF image\n", file_name); break;
    default: break;
    }
    return ret;
}
            ...
            if (*token == '-') { /* GNU style */
                ...
                if (strcmp(nextArg, "default") == 0)
                  ...
                  (void) lame_set_strict_ISO(gfp, MDB_MAXIMUM);
                    ...
                    printf("unknown buffer constraint '%s'\n", nextArg);
                    return -1;
                }
                ...
                T_ELIF("tg")
                    ...
                    if (ret != 0) {
                        if (0 == ignore_tag_errors) {
                            if (ret == -1) {
                                printf("Unknown ID3v1 genre number: '%s'.\n", nextArg);
                                return -1;
                            }
                            else if (ret == -2) {
                                if (id3tag_mode == ID3TAG_MODE_V1_ONLY) {
                                    printf("Unknown ID3v1 genre: '%s'.\n", nextArg);
                                    return -1;
                                }
                                else if (id3tag_mode == ID3TAG_MODE_V2_ONLY) {
                                    ...
                                    if (global_ui_config.silent < 9) {
                                        printf("Unknown ID3v1 genre: '%s'.  Setting ID3v1 genre to 'Other'\n", nextArg);
                                    }
                                }
                            }
                            else {
                ...
                T_ELIF("tv")
                    ...
                    if (id3_tag(gfp, 'v', id3_tenc, nextArg)) {
                        if (global_ui_config.silent < 9) {
                            printf("Invalid field value: '%s'. Ignored\n", nextArg);
                        }
                    }
...
//        usage(Console_IO.Console_fp, ProgramName);
    ...
    if (nogap && lame_get_bWriteVbrTag(gfp) && nogap_tags == 0) {
        printf("Note: Disabling VBR Xing/Info tag since it interferes with --nogap\n");
    ...
    if (is_mpeg_file_format(global_reader.input_format)) {
        printf("Error: libmp3lame not compiled with mpg123 *decoding* support \n");
        return -1;
    }
#endif
    ...
    if (lame_get_free_format(gfp)) {
        if (lame_get_brate(gfp) < 8 || lame_get_brate(gfp) > 640) {
            ...
            printf("with the -b <bitrate> option\n");
            return -1;
        }
    }
    if (num_nogap != NULL)
