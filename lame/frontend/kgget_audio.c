...
{
    unsigned int  const hh = bytes[0];
    unsigned int const hl = bytes[1];
    unsigned int const lh = bytes[2];
    unsigned int const ll = bytes[3];
...
read_ieee_extended_high_low(FILE * fp)
{
    ...
    fread(bytes, 1, 10, fp);
    {
        int const s = (bytes[0] & 0x80);
        int const e_h = (bytes[0] & 0x7F);
        int const e_l = bytes[1];
        int e = (e_h << 8) | e_l;
        unsigned int  const hm = uint32_high_low(bytes + 2);
        unsigned int  const lm = uint32_high_low(bytes + 6);
        double  result = 0;
        if (e != 0 || hm != 0 || lm != 0) {
            if (e == 0x7fff) {
                result = HUGE_VAL;
...
read_16_bits_low_high(FILE * fp)
{
    ...
    fread(bytes, 1, 2, fp);
    {
        int const low = bytes[0];
        int const high = (signed char) (bytes[1]);
        return (high << 8) | low;
    }
}
...
read_32_bits_low_high(FILE * fp)
{
    ...
    {
        int const low = bytes[0];
        int const medl = bytes[1];
        int const medh = bytes[2];
        int const high = (signed char) (bytes[3]);
...
read_16_bits_high_low(FILE * fp)
{
    ...
    fread(bytes, 1, 2, fp);
    {
        int const low = bytes[1];
        int const high = (signed char) (bytes[0]);
        return (high << 8) | low;
    }
}
...
read_32_bits_high_low(FILE * fp)
{
    ...
    fread(bytes, 1, 4, fp);
    {
        int const low = bytes[3];
        int const medl = bytes[2];
        int const medh = bytes[1];
        int const high = (signed char) (bytes[0]);
        return (high << 24) | (medh << 16) | (medl << 8) | low;
    }
}
...
open_snd_file(lame_t gfp, char const *inPath)
{
...

    {
        ...
        if (gs_pSndFileIn == NULL) {
            ...
            if (global_ui_config.silent < 10) {
                printf("Could not open sound file \"%s\".\n", lpszFileName);
            }
            exit(1);
        }
...
lame_decode_initfile(FILE * fd, mp3data_struct * mp3data, int *enc_delay, int *enc_padding)
{
    ...
    if (aid_header) {
        ...
        if (global_ui_config.silent < 9) {
            printf("Album ID found.  length=%i \n", aid_header);
        }
